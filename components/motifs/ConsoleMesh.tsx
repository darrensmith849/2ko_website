    ...
    >
      ...
      <g>
        ...
        <rect x="290" y="29" width="60" height="18" rx="9"
              fill="rgb(var(--motif-fg) / 0.04)" stroke="var(--tint)" strokeOpacity="0.5" />
        ...
        <text x="383" y="41" textAnchor="middle" fill="rgb(var(--motif-fg) / 0.75)"
              fontFamily="var(--font-geist-mono, monospace)" fontSize="9"
              letterSpacing="0.15em">4 · ALERTS</text>
        <rect x="416" y="29" width="50" height="18" rx="9"
              fill="rgb(var(--motif-fg) / 0.04)" stroke="rgb(var(--motif-fg) / 0.18)" />
        <text x="441" y="41" textAnchor="middle" fill="rgb(var(--motif-fg) / 0.75)"
              fontFamily="var(--font-geist-mono, monospace)" fontSize="9"
      ...
      {EDGES.map(([a, b, hot], i) => {
        ...
        return (
          <line
            ...
            stroke={hot ? "var(--tint)" : "rgb(var(--motif-fg) / 0.16)"}
      ...
      {NODES.map((n, i) => (
        <g key={`n-${i}`}>
          ...
          <circle
            ...
            fill={n.active ? "var(--tint)" : "rgb(var(--motif-fg) / 0.65)"}
      ...
      <g className="float-a" style={{ transformOrigin: "110px 165px" }}>
        <rect x="42" y="138" width="138" height="56" rx="10"
              fill="url(#cm-panel)" stroke="rgb(var(--motif-fg) / 0.14)" />
        <text x="52" y="156" fill="rgb(var(--motif-fg) / 0.55)"
              fontFamily="var(--font-geist-mono, monospace)"
              fontSize="9" letterSpacing="0.16em">THROUGHPUT</text>
        <text x="52" y="180" fill="rgb(var(--motif-fg) / 0.95)"
      ...
      <g className="float-b" style={{ transformOrigin: "395px 180px" }}>
        <rect x="318" y="138" width="156" height="84" rx="10"
              fill="url(#cm-panel)" stroke="rgb(var(--motif-fg) / 0.14)" />
        <text x="330" y="156" fill="rgb(var(--motif-fg) / 0.55)"
              fontFamily="var(--font-geist-mono, monospace)"
        ...
        <line x1="330" y1="210" x2="455" y2="210" stroke="rgb(var(--motif-fg) / 0.18)" />
      </g>
      ...
      <g className="float-c" style={{ transformOrigin: "210px 380px" }}>
        ...
        <rect x="158" y="363" width="76" height="5" rx="2.5" fill="rgb(var(--motif-fg) / 0.75)" />
        <rect x="158" y="372" width="120" height="3" rx="1.5" fill="rgb(var(--motif-fg) / 0.22)" />
        ...
        {/* Row 2 */}
        <circle cx="146" cy="392" r="4" fill="rgb(var(--motif-fg) / 0.4)" />
        <rect x="158" y="388" width="62" height="5" rx="2.5" fill="rgb(var(--motif-fg) / 0.55)" />
        <rect x="158" y="397" width="104" height="3" rx="1.5" fill="rgb(var(--motif-fg) / 0.18)" />
        <rect x="306" y="387" width="36" height="12" rx="6"
              fill="rgb(var(--motif-fg) / 0.04)" stroke="rgb(var(--motif-fg) / 0.25)" />
        <text x="324" y="396" textAnchor="middle" fill="rgb(var(--motif-fg) / 0.65)"
              fontFamily="var(--font-geist-mono, monospace)"
